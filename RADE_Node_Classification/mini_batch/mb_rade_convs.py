...
class BatchGraphCache:
    """
    Batch-local cache derived from the CLEAN *batch-induced* subgraph (no self-loops).
    ...
    """
    edge_index_clean_dir: torch.Tensor  # [2, E_dir], no self-loops
    num_nodes: int
    deg_clean: torch.Tensor             # [N]
    comp_size: torch.Tensor             # [N]

    @staticmethod
    def from_edge_index(edge_index_clean: torch.Tensor, num_nodes: int) -> "BatchGraphCache":
        ...
        deg.index_add_(0, col, ones)  # in-degree (== degree if undirected with both directions)

        comp = (num_nodes - 1.0 - deg).clamp(min=0.0)
        return BatchGraphCache(edge_index_clean_dir=edge_index_clean_dir, num_nodes=num_nodes, deg_clean=deg, comp_size=comp)

    # ---------- Complement ops (batch-local) ----------

    def complement_mean_unweighted(self, x: torch.Tensor) -> torch.Tensor:
        """
        μ_i = mean_{j∈C_batch(i)} x_j
        computed exactly in O(E_batch + N_batch).
        """
        N = self.num_nodes
        row, col = self.edge_index_clean_dir.to(x.device)
        neigh_sum = _scatter_add_rows(x[row], col, N)          # sum over N_batch(i)
        total_sum = x.sum(dim=0, keepdim=True)                 # sum over all batch nodes
        comp_sum = total_sum - x - neigh_sum                   # sum over C_batch(i)
    ...
    def complement_sum_weighted(self, x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
        """
        S_i = sum_{j∈C_batch(i)} w_j x_j, where w depends only on node j.
        """
        N = self.num_nodes
        row, col = self.edge_index_clean_dir.to(x.device)
        ...
        neigh_wx = _scatter_add_rows(wx[row], col, N)     # sum_{j∈N(i)} w_j x_j
        comp_wx = total_wx - wx - neigh_wx                # sum_{j∈C(i)} w_j x_j
        return comp_wx

    def complement_mean_weighted(self, x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
        """
        μ_i = (∑_{j∈C_batch(i)} w_j x_j) / (∑_{j∈C_batch(i)} w_j), weights depend only on j.
        """
        N = self.num_nodes
        row, col = self.edge_index_clean_dir.to(x.device)
...
# ---------- Delta-method terms (same formulas, but batch degrees/complements) ----------
...
class RADEGCNConvMB(nn.Module):
    """
    Mini-batch version of RADEGCNConv (batch-local).
    ...
    def forward(
        ...
        *,
        ...
        assert self.graph_cache is not None, "Mini-batch: model must set_graph(...) for each batch (or do it in forward)."
        ...
        if edge_index_keep is None:
            row, col = edge_index_clean[0], edge_index_clean[1]
            # robustly remove any self-loops, then add self-loops
            mask = row != col
            row, col = row[mask], col[mask]
            ...
            # RADE-IC inference correction (batch-local; used only in eval)
            if self.ic_mode and (not self.training) and (float(q) > 0.0):
                deg_clean = self.graph_cache.deg_clean.to(x.device)
                comp_size = self.graph_cache.comp_size.to(x.device)

                tB = _delta_E_inv_sqrt_caseB(deg_clean, comp_size, p=float(p), q=float(q)).to(x.device, dtype=x.dtype)
                comp_wx = self.graph_cache.complement_sum_weighted(x, w=tB)
                out = out + (float(q) * tB).unsqueeze(1) * comp_wx
        ...
        # Build augmented degree using keep + add + self
        ar = torch.arange(N, device=x.device)
        ...
        Gamma_k = inv_sqrt_aug[row_k] * inv_sqrt_aug[col_k]              # [E_keep]
        ...
        E_Gamma_k = (1.0 - float(p)) * tA[row_k] * tA[col_k]
        ...
        if self.correct_self_loop:
            E_inv = _delta_E_inv_self(deg_clean, comp_size, p=float(p), q=float(q)).to(x.device, dtype=x.dtype)
...
class RADEGINConvMB(nn.Module):
    """
    Mini-batch version of RADEGINConv (batch-local complement).

    - RADE (ic_mode=False): centered additions (mu over C_batch(i))
    - RADE-IC (ic_mode=True):
        * Train: uncentered adds
